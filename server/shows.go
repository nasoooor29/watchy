...
func getMetadataForShow(showDir string) (map[string]any, error) {
	metadataPath := filepath.Join(models.ENV.TvDir, showDir, "data.json")
	ignoredPath := filepath.Join(models.ENV.TvDir, showDir, ".hide-from-list")
...
func (s *Server) ShowsPage(w http.ResponseWriter, r *http.Request) {
	...
	dirs, err := os.ReadDir(models.ENV.TvDir)
	if err != nil {
		slog.Error("", "err", err)
	...
	for _, dir := range dirs {
		...
		meta, err := getMetadataForShow(dir.Name())
		if err != nil {
	...
	err = s.render(w, r, http.StatusOK, "home.html", map[string]any{
		"TotalSeries": 5,
		"Shows":       shows,
	})
	if err != nil {
...
func (s *Server) GetShowCover(w http.ResponseWriter, r *http.Request) {
	coverPath := filepath.Join(models.ENV.TvDir, r.PathValue("ShowName"), "cover.jpg")
